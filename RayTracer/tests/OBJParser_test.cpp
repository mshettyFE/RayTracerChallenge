...
"f 1 2 3 4 5\n",false);
    ...
    auto g = p.emit();
    const Shape* first_group = g->get_child(0);
    EXPECT_EQ(first_group->get_total_children(), 3);
    auto  points_triangle_one = dynamic_cast<const Triangle*>(first_group->get_child(0))->get_points();
    ...
    auto  points_triangle_two = dynamic_cast<const Triangle*>(first_group->get_child(1))->get_points();
    ...
    auto  points_triangle_three = dynamic_cast<const Triangle*>(first_group->get_child(2))->get_points();
...
"f 1 3 4\n",false
    ...
    auto g = p.emit();   
    const Shape* first_group = g->get_child(0);
    const Shape* second_group = g->get_child(1);
    EXPECT_EQ(g->get_total_children(), 2);
    EXPECT_EQ(first_group->get_total_children(),1);
    auto  points_triangle_one = dynamic_cast<const Triangle*>(first_group->get_child(0))->get_points();
    EXPECT_EQ(points_triangle_one[0],vertices[1]);
    EXPECT_EQ(points_triangle_one[1],vertices[2]);
    EXPECT_EQ(points_triangle_one[2],vertices[3]);
    EXPECT_EQ(second_group->get_total_children(),1);
    auto  points_triangle_two = dynamic_cast<const Triangle*>(second_group->get_child(0))->get_points();
    EXPECT_EQ(points_triangle_two[0],vertices[1]);
    EXPECT_EQ(points_triangle_two[1],vertices[3]);
    EXPECT_EQ(points_triangle_two[2],vertices[4]);
}
