...
CollisionInfo::CollisionInfo(std::unique_ptr<Impact> impt, const Ray& ray, const std::vector<Impact>& all_hits){
    this->impact = std::move(impt);
    this->pnt =  ray.position(this->impact->get_t());
    this->eye = -1.0*ray.get_direction();
    this->normal = this->impact->get_obj()->normal_at(pnt);
    if(eye.dot(normal) < 0){
        this->inside = true;
...
const Impact CollisionInfo::get_impact() const{return *impact;}
